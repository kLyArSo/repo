...
void	*entry_point(void	*ptr)
{
	...
	while (1)
	{
		...
		print_status("has taken a fork", data);
		...
		print_status("is sleeping", data);
		my_sleep(data->game_args->time_to_sleep * 1000);
		print_status("is thinking", data);
	}
	return (NULL);
}
...
void	print_status(char	*str, t_philo_data	*data)
{
	...
	printf("[%ld ms] %d %s.\n",
		(get_current_time_micro_seconds() - data->checker->start_time)
		/ 1000, x + 1, str);
	pthread_mutex_unlock(&data->checker->lock_1);
}
